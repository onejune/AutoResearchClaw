...
class HardwareMonitor:
    """硬件资源监控器 - 监控和管理系统 GPU/CPU 资源"""

    def __init__(self) -> None:
        """初始化硬件监控器，检测所有可用设备"""
        self.gpus: List[DeviceInfo] = self._detect_gpus()
        self.cpu: DeviceInfo = self._detect_cpu()
    ...
    def get_idle_gpus(self, max_util: float = 10.0, max_mem_mb: float = 500) -> List[int]:
        ...
        for gpu in self.gpus:
            if gpu.utilization < max_util and gpu.memory_used_mb < max_mem_mb:
                idle_gpus.append(gpu.device_id)

        return idle_gpus

    def is_cpu_idle(self, max_util: float = 70.0) -> bool:
        """
        检查 CPU 是否空闲
        ...
        """
        self.refresh()
        return self.cpu.utilization < max_util

    def get_best_device(self, min_mem_mb: float = 4096) -> Tuple[str, Optional[int]]:
        """
        获取最佳可用设备

        Args:
            min_mem_mb: 最小显存需求 (MB)

        Returns:
            ("gpu", gpu_idx) 或 ("cpu", None)
        """
        self.refresh()

        # 优先找空闲 GPU
        for gpu in self.gpus:
            if gpu.memory_available_mb >= min_mem_mb:
                return ("gpu", gpu.device_id)

        # 如果没有满足条件的 GPU，返回 CPU
        return ("cpu", None)
    ...
    def select_device(
        ...
        # 如果环境变量 CUDA_VISIBLE_DEVICES 已设置，直接用指定 GPU，跳过利用率检测
        import os
        cuda_env = os.environ.get("CUDA_VISIBLE_DEVICES", "")
        if cuda_env.strip() and cuda_env.strip() not in ("-1", "NoDevFiles") and self.gpus:
            try:
                forced_id = int(cuda_env.strip().split(",")[0])
                for gpu in self.gpus:
                    if gpu.device_id == forced_id:
                        selected_device = gpu
                        reason = f"CUDA_VISIBLE_DEVICES={cuda_env} 强制指定 GPU {forced_id}"
                        break
                if selected_device is None and self.gpus:
                    selected_device = self.gpus[0]
                    reason = f"CUDA_VISIBLE_DEVICES={cuda_env} 但未找到对应 GPU，使用 GPU 0"
            except (ValueError, IndexError):
                pass

        if selected_device is None and prefer_gpu and self.gpus:
            # 优先 GPU：找第一个满足条件的
            for gpu in self.gpus:
