...
def main():
    ...
    # 调试模式：分层采样（按 domain 比例保持，避免小 domain 样本过少）
    if os.environ.get("DEBUG_SAMPLE"):
        import numpy as np
        sample_ratio = float(os.environ.get("DEBUG_SAMPLE", "0.01"))
        
        def stratified_sample(dataset, ratio):
            """按 domain_indicator 分层采样"""
            indices = []
            # 获取所有 domain_indicator
            domains = dataset.domains  # IVRDataset 有 domains 属性
            
            # 按 domain 分层采样
            for d in np.unique(domains):
                d_indices = np.where(domains == d)[0]
                n_sample = max(int(len(d_indices) * ratio), 50)  # 每个 domain 至少 50 个样本
                if n_sample > len(d_indices):
                    n_sample = len(d_indices)
                sampled = np.random.choice(d_indices, n_sample, replace=False)
                indices.extend(sampled.tolist())
            
            return torch.utils.data.Subset(dataset, indices)
        
        train_ds = stratified_sample(train_ds, sample_ratio)
        val_ds = stratified_sample(val_ds, sample_ratio)
        logger.info(f"[DEBUG] 分层采样 {sample_ratio*100}% 数据：训练 {len(train_ds)}，验证 {len(val_ds)}")
