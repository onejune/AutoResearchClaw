...
# =============================================================================
# FDAM Loss (Weibull Delay + Online Label Correction)
# =============================================================================

def fdam_loss(outputs: Dict[str, torch.Tensor],
              targets: Dict[str, torch.Tensor],
              eps: float = 1e-7) -> torch.Tensor:
    """
    FDAM loss: Weibull delay distribution + online label correction.

    Uses pretrained tn/dp model (same as ES-DFM) for importance weights.
    Weibull soft label correction for non-converted samples.

    Formula:
    - Weibull survival: S(e) = exp(-(e/λ)^k)
    - soft_label for non-converted: S(elapsed)  (probability of future conversion)
    - pos_weight = 1 + dp_prob
    - neg_weight = tn_prob * (1 + dp_prob)
    - loss = pos_weight * BCE(logits, 1) * z + neg_weight * BCE(logits, soft_label) * (1-z)
    """
    x = outputs["logits"].squeeze(-1)
    ...
    # Weibull parameters
    k = F.softplus(log_k_raw) + 0.1      # shape > 0.1
    lamb = F.softplus(log_lamb_raw) + 1.0  # scale > 1 second

    # Weibull survival: P(delay > elapsed) = exp(-(elapsed/lambda)^k)
    survival = torch.exp(-torch.pow(elapsed / (lamb + eps), k))

    # Soft label for non-converted: survival probability
    soft_label = survival.clamp(0.0, 1.0)

    # Importance weights (same structure as ES-DFM)
    pos_weight = 1 + dp_prob
    neg_weight = tn_prob * (1 + dp_prob)

    # Loss
    pos_loss = F.binary_cross_entropy_with_logits(x, torch.ones_like(z), reduction='none')
    neg_loss = F.binary_cross_entropy_with_logits(x, soft_label.detach(), reduction='none')
