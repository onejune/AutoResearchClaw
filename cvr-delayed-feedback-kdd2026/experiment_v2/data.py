...
class DataDF:
    ...
    def add_fdam_data(self, cut_size: int) -> 'DataDF':
        """
        FDAM data processing: same as ES-DFM structure, but adds elapsed time.

        Delayed positives are duplicated at pay_ts (same as ES-DFM).
        Elapsed time is stored for Weibull soft label computation.

        Args:
            cut_size: observation window in seconds
        """
        # Same data structure as ES-DFM
        return self.add_esdfm_cut_fake_neg(cut_size)
...
def get_streaming_data(
    ...
    for hour in range(start_hour, end_hour):
        ...
        if method == "Vanilla":
            ...
            train_data = hour_data.add_fdam_data(ob_win)
        else:
