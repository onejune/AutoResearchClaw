...
class CriteoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data: DataDF, cat_hash_sizes: Tuple[int, ...]):
        ...
        # Elapsed time: current_time - click_ts (for Weibull soft label)
        # Use max click_ts in batch as proxy for current time
        max_ts = data.click_ts.max()
        elapsed = np.maximum(max_ts - data.click_ts, 1.0).astype(np.float32)
        self.elapsed = torch.tensor(elapsed, dtype=torch.float32)
...
class Trainer:
    ...
    def run_all_methods(self, data: DataDF) -> Dict[str, Dict[str, float]]:
        ...
        for method in methods:
            ...
            aux_models = None
            if method in ["ES-DFM", "DEFUSE", "FDAM"]:
