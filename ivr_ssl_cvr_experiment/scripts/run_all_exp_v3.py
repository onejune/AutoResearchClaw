#!/usr/bin/env python3
"""
data_raw 全实验 V3：baseline + MLoRA + 对比学习（共10个配置）
修复 augmentation 的浅拷贝 bug
"""
...
def main():
    ...
    with open(f'{RESULTS_DIR}/data_raw_all_exp_v3.json', 'w') as f:
...
{sep}
""" + "\n".join(rows) + f"\n{overall_row}\n"
    with open(f'{RESULTS_DIR}/data_raw_all_exp_v3.md', 'w') as f:
        f.write(md)
    ...
    print(sep)
    print(overall_row)
    print(f"\n结果已保存: {RESULTS_DIR}/data_raw_all_exp_v3.json")
