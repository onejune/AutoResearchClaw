#!/usr/bin/env python3
"""
按 business_type 分组评估 - 续跑版（跳过已完成的 baseline 和 bt_contrastive cw=0.05）
"""
...
if __name__ == '__main__':
    ...
    # 已完成的结果（从 eval_by_bt.log 读取）
    completed_results = [
        {
            'model': 'baseline', 'cl_weight': 0.1, 'overall_auc': 0.8321,
            'bt_aucs': {'aecps': 0.7387, 'aedsp': 0.6419, 'aerta': 0.7083,
                        'lazada_cps': 0.7786, 'lazada_rta': 0.7474, 'shein': 0.5150, 'shopee_cps': 0.7340}
        },
        {
            'model': 'bt_contrastive', 'cl_weight': 0.05, 'overall_auc': 0.8315,
            'bt_aucs': {'aecps': 0.7332, 'aedsp': 0.6815, 'aerta': 0.6950,
                        'lazada_cps': 0.7712, 'lazada_rta': 0.7488, 'shein': 0.5169, 'shopee_cps': 0.7341}
        },
    ]
    ...
    new_results = []
    for model_name, cl_weight in remaining_experiments:
        print(f"\n{'='*60}")
    ...
    header = f"{'Model':<32} {'Overall':<10}" + "".join(f"{b:<14}" for b in bt_names)
    print(header)
    print("-" * len(header))
    for r in all_results:
        label = f"{r['model']}(cw={r['cl_weight']})"
        row = f"{label:<32} {r['overall_auc']:<10}"
        for b in bt_names:
            v = r['bt_aucs'].get(b)
