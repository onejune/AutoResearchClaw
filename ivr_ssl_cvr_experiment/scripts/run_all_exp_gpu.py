#!/usr/bin/env python3
"""
data_raw 全实验 GPU 版：baseline + MLoRA + 对比学习（共10个配置）
基于 run_all_exp_v3.py，改动：
1. DEVICE 强制设为 'cuda'
2. MLoRA 的 LoRALinear.forward 改成用 einsum
"""
...
class BaselineCVR(nn.Module):
    ...
    def __init__(self, str_features, num_features, str_vocab_sizes,
        ...
        # 保存输出维度和 repr 提取器
        self.repr_net = nn.Sequential(*self.net[:-1])  # [B, 128]
...
class LoRALinear(nn.Module):
    def __init__(self, in_f, out_f, n_domains, rank=8):
        super().__init__()
        self.s = nn.Linear(in_f, out_f, bias=True)
        self.A = nn.Embedding(n_domains, rank * in_f)
        self.B = nn.Embedding(n_domains, rank * out_f)
        self.r = rank
        self.i = in_f
        self.o = out_f
    ...
    def forward(self, x, d):
        base = self.s(x)
        a = self.A(d).view(-1, self.r, self.i)
        b = self.B(d).view(-1, self.r, self.o)
        lora = torch.einsum('bri,bro->bo', a, b)
        return base + lora
...
class MLoRACVR(nn.Module):
    ...
    def forward(self, x_dict, domain_ids, user_ids=None, temperature=0.1):
        ...
        result = {'pred': pred}
        if self.training and user_ids is not None:
...
def train_bt_contrastive(model, train_dl, device, cl_weight=0.1, temperature=0.1, epochs=1, lr=1e-3):
    """BT Contrastive: 同一 business_type 的样本拉近"""
    opt = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)
    model.train()
    for ep in range(epochs):
        for bi, (features, labels, bt_ids, user_ids) in enumerate(train_dl):
            ...
            reprs = model.get_repr(features)
            reprs = F.normalize(reprs, dim=-1)

            bt_sim = reprs @ reprs.T
...
def train_augment_contrastive(model, train_dl, device, cl_weight=0.1, temperature=0.1,
    ...
    """Augment Contrastive: dropout 字符串特征，两个 view 做对比"""
    opt = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)
    model.train()
    str_features = list(model.str_features)
    num_features = list(model.num_features)
    ...
    for ep in range(epochs):
        for bi, (features, labels, bt_ids, user_ids) in enumerate(train_dl):
            ...
            # View2: dropout 部分字符串特征（保留数值特征）
            feat_drop = np.random.rand(B, n_str) < dropout_rate
            aug_str = {}
            for i, f in enumerate(str_features):
                # mask 为 True 的位置替换成0（hash=0 通常是 padding）
                masked = features[f].clone()
                masked[torch.from_numpy(feat_drop[:, i]).to(device)] = 0
                aug_str[f] = masked

            # 合并数值特征
            aug_feat = {**aug_str, **{f: features[f] for f in num_features}}
            repr2 = model.get_repr(aug_feat)
            repr2 = F.normalize(repr2, dim=-1)

            # 对比 loss: view1 vs view2（同一样本的两个 view 应该接近）
            sim = repr1 @ repr2.T
            sim = sim / temperature
            labels_cl = torch.arange(B, device=device)
            cl_loss = F.cross_entropy(sim, labels_cl)
...
def main():
    print("="*60)
    print("data_raw 全实验 GPU 版（共10个配置）")
    print("="*60)
    ...
    experiments = [
        ...
        ('bt_contrastive_cw0.1', lambda: BaselineCVR(str_features, num_features, str_vocab_sizes, 32, [512,256,128]),
         lambda m: train_bt_contrastive(m, train_dl, DEVICE, cl_weight=0.1, epochs=1), False,
         {'cl_type': 'bt_contrastive', 'cl_weight': 0.1}),
        ('bt_contrastive_cw0.05', lambda: BaselineCVR(str_features, num_features, str_vocab_sizes, 32, [512,256,128]),
         lambda m: train_bt_contrastive(m, train_dl, DEVICE, cl_weight=0.05, epochs=1), False,
         {'cl_type': 'bt_contrastive', 'cl_weight': 0.05}),
        ('augment_contrastive_cw0.1', lambda: BaselineCVR(str_features, num_features, str_vocab_sizes, 32, [512,256,128]),
        ...
        ('mlora_rank4', lambda: MLoRACVR(str_features, num_features, str_vocab_sizes, 32, 128, n_domains, 4, [256,128]),
        ...
        ('mlora_rank4_user_cl0.1', lambda: MLoRACVR(str_features, num_features, str_vocab_sizes, 32, 128, n_domains, 4, [256,128]),
         lambda m: train_mlora(m, train_dl, DEVICE, cl_weight=0.1, epochs=1), True,
         {'cl_type': 'user_contrastive', 'cl_weight': 0.1}),
    ...
    with open(f'{RESULTS_DIR}/data_raw_all_exp_gpu.json', 'w') as f:
...
{sep}
""" + "\n".join(rows) + f"\n{overall_row}\n"
    with open(f'{RESULTS_DIR}/data_raw_all_exp_gpu.md', 'w') as f:
        f.write(md)
    ...
    print(sep)
    print(overall_row)
    print(f"\n结果已保存: {RESULTS_DIR}/data_raw_all_exp_gpu.json")
