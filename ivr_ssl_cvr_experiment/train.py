...
class Config:
    # 数据
    data_dir: str = '/mnt/workspace/open_research/autoresearch/ivr_ssl_cvr/data/'
    
    # 模型
    model_name: str = 'baseline'  # baseline / contrastive / bt_contrastive
    embed_dim: int = 32
...
class Trainer:
    ...
    def train_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        for batch in tqdm(train_loader, desc='Training', leave=False):
            ...
            elif 'z' in outputs and hasattr(self.model, 'contrastive_loss'):
                if self.config.model_name == 'bt_contrastive':
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline', 
                        choices=['baseline', 'contrastive', 'bt_contrastive', 'augment_contrastive'])
    ...
    # 创建数据集
    train_dataset = IVRDataset(train_df, features)
    val_dataset = IVRDataset(val_df, features)
    ...
    logger.info(f'PCOC: {metrics["pcoc"]:.2f}')
