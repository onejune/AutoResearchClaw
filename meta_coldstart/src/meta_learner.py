...
1. MAML   - Model-Agnostic Meta-Learning（二阶梯度，Finn et al. 2017）
2. FOMAML - 一阶近似 MAML（去掉二阶梯度，计算更高效）
3. ANIL   - Almost No Inner Loop（只在 head 层做 inner loop）
4. ProtoNet - 原型网络（适合极少样本场景）
"""
import copy
import logging
...
def inner_loop(model: PurchaseModel, support_loader: DataLoader,
    ...
    for step in range(inner_steps):
        for X, y in support_loader:
            ...
            # 手动更新参数（保留计算图用于 outer loop）
            # 使用 param.copy_() 来就地更新，这样可以保持参数的梯度连接
            for param, grad in zip(adapted.parameters(), grads):
                if grad is not None:
                    param.data.copy_(param.data - inner_lr * grad)
            break  # 每步只用一个 batch
        ...
        for epoch in range(cfg.meta_epochs):
            ...
            for task in batch_tasks:
                ...
                for X, y in query_loader:
                    ...
                    outer_loss = outer_loss + adapted.loss(X, y)
                    batch_count += 1
                    # 如果 query set 很大，限制使用的 batch 数以控制计算量
                    if batch_count >= 3:  # 每个 task 最多用 3 个 batch
                        break

            outer_loss = outer_loss / len(batch_tasks)
            if batch_count > 0:
                outer_loss = outer_loss / batch_count  # 平均到每个 batch
            outer_loss.backward()
...
class ProtoNet:
    ...
    def _episode_loss(self, task: Task, features: List[str]):
        ...
        # 距离：负类距离 - 正类距离 → logit
        dist_pos = torch.cdist(query_emb.unsqueeze(0),
                               proto_pos.unsqueeze(0).unsqueeze(0)).squeeze()
        dist_neg = torch.cdist(query_emb.unsqueeze(0),
                               proto_neg.unsqueeze(0).unsqueeze(0)).squeeze()
